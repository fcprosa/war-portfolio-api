// /api/kalshi.js — thin HTTP handler for Kalshi prediction market data
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      positions = (state?.predictionMarkets || []).filter(p => p.platform === 'kalshi');
    }

    const data = await fetchKalshiState(positions);
    return res.status(200).json(data);
  } catch (err) {
    console.error('[kalshi] handler failed:', err.message);
